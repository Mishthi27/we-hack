...
async def generate_quiz():
    ...
    new_mcqs = generate_mcqs_from_text(extracted_text, num_questions=5)

    if "latest" not in quiz_questions_db:
        quiz_questions_db["latest"] = new_mcqs
    else:
        quiz_questions_db["latest"].extend(new_mcqs)

    return {"message": "MCQs generated successfully", "questions": quiz_questions_db["latest"]}
